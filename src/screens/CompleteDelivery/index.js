...
const CompleteDelivery = ({ route, navigation }) => {
  ...
  const [cdCond, setCdCond] = useState(initCDCondition);
  ...
  const onPressSubmitTask = async () => {
    Services.getCurrentLocation().then((loc) => {
      ...
      const newCD = cdCond.filter((item) => item.active === true);
      ...
      const task = {
        ...
        status: newCD[0].title.toUpperCase(),
        ...
        Services.updateServiceOrder(state.id, task).then((res) => {
          ...
          if (!res) {
            ...
            let temp = {
              ...
              remarks: 'generated by complete delivery',
  ...
  const setNewCDSelect = (item) => {
    const newD = cdCond.slice();
    newD.forEach((i) => (i.active = false));
    newD[item.id - 1].active = true;
    setCdCond(newD);
  };
  ...
  return (
    <>
      ...
      >
        ...
        />
        <Header title={'COMPLETE DELIVERY'} navigation={navigation} />
