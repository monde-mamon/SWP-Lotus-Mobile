  ...
  const onPressSubmitTask = () => {
    Services.getCurrentLocation().then((loc) => {
      ...
      let temp = {
        ...
        remarks: 'generated by return-receive',
  ...
  return (
    <>
      ...
      >
        ...
        />
        <Header title={'RETURN'} navigation={navigation} />
        <ScrollView showsVerticalScrollIndicator={false}>
          <>
            ...
            <Card title={fetchLanguage[lang].receivingdepartment}>
              {departmentCond.map((item) => (
                <TouchableOpacity
                  key={item.id.toString()}
                  style={{
                    width: '100%',
                  ...
                  disabled={!newTask}
                  onPress={() => setNewReceivingDept(item)}
                >
                  {item.active ? (
                    <Ionicons
                      name="radio-button-on-outline"
